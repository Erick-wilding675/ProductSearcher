...
>
> **Status: aprovado e executado.** Este documento é um registro datado, mantido como
> está para preservar os números que sustentaram as decisões. As contagens da seção 1
> descrevem o banco de 09/08/2026 e **não** o catálogo atual. O desfecho de cada frente e
> de cada decisão está na seção 7, no fim do arquivo. O estado corrente do esquema está em
...
**Autenticação:** `client_credentials` funciona, com token de 6h, **sem navegador**.
...
| `GET /items/{id}` | 403 | Bloqueado para token de app, e não precisamos |
...
## 7. Desfecho (registrado em 2026-09-19)

O plano foi aprovado e executado. O que aconteceu com cada item:

### Frentes

| Frente | Desfecho |
| --- | --- |
| F1, enriquecimento pela API | Executada. Virou o [ADR-0009](../adr/0009-enriquecimento-pela-api-e-identidade-do-produto.md), que também fechou a identidade do produto e a fusão de variantes de cor |
| F2, ofertas múltiplas e `stores.url` | Executada. O catálogo passou a ter várias ofertas por produto, com concorrência de preço real |
| F3, schema contra realidade | Executada. `searches` passou a receber escrita e os atributos foram reconciliados |
| F4, pendências documentais | Executada, e revisada de novo nesta data |
| F5, extensão em modo desenvolvedor | Executada. O procedimento está em [`../extension/README.md`](../extension/README.md) |
...
Duas coisas apareceram depois e estão registradas em outro lugar, porque nasceram de
execução e não de levantamento:

- **Recarregar o seed apaga a rotulagem de `use_case`**, porque o upsert de
  `product_specs` substituía `attributes` inteiro. Corrigido para merge, e o passo de
  re-rotulagem entrou no runbook (ADR-0010 e ADR-0011).
- **Ofertas com preço implausível contaminam preço exibido, ranking e comparação.** O
  levantamento viu o problema oposto (uma oferta por produto) e não este. Tratado no
