...
const SORTS_VALIDOS: SortOption[] = [
  "relevance",
  "price_asc",
  "price_desc",
  "name",
...
const RANKS_VALIDOS: RankByOption[] = [
  "relevance",
  "price",
  "brand",
  "spec",
...
function formatPreferenceSpecKey(
  key: string
): string {
...
function SearchPageContent() {
  ...
  const sortUrl: SortOption =
    SORTS_VALIDOS.includes(
  ...
  const rankByParam =
    searchParams.get("rank_by");

  const rankByUrl: RankByOption =
    RANKS_VALIDOS.includes(
  ...
  const rankBrandUrl =
    searchParams.get("rank_brand") ?? "";

  const rankSpecUrl =
    searchParams.get("rank_spec") ?? "";

  const rankSpecValueUrl =
    searchParams.get("rank_spec_value") ?? "";

  const pageUrl = Math.max(
    1,
    Number(
  ...
  const [category, setCategory] =
    useState(categoryUrl);

  const [priceMax, setPriceMax] =
    useState(priceMaxUrl);
   ...
   */
  const [rankByDraft, setRankByDraft] =
    useState<RankByOption>(rankByUrl);

  const [rankBrandDraft, setRankBrandDraft] =
    useState(rankBrandUrl);

  const [rankSpecDraft, setRankSpecDraft] =
    useState(rankSpecUrl);

  const [
    rankSpecValueDraft,
    setRankSpecValueDraft,
  ] = useState(rankSpecValueUrl);
      ...
      () =>
        categorias.map(
          (categoryItem) => ({
            ...
            label:
              `${categoryItem.name} (${categoryItem.product_count})`,
          })
        ),
      [categorias]
    );
      ...
      () =>
        marcas.map(
          (brandItem) => ({
            ...
            label:
              `${brandItem.name} (${brandItem.product_count})`,
          })
        ),
      [marcas]
    );
  ...
  function handleRankByChange(
    novoRank: RankByOption
  ) {
  ...
  function handleRankBrandChange(
    novaMarca: string
  ) {
    setRankBrandDraft(
      novaMarca
  ...
  function handleRankSpecValueChange(
    novoValor: string
  ) {
    setRankSpecValueDraft(
      novoValor
    ...
    rankByUrl === "brand"
      ...
      : rankByUrl === "spec"
        ...
        ? `${
            specOptions.find(
              (spec) =>
                spec.key === rankSpecUrl
            )?.label
            ?? formatPreferenceSpecKey(
              rankSpecUrl
            )
